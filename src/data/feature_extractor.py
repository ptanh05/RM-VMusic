...
Feature Extraction Pipeline for Dataset V3 (N = 5,569).
Extracts physical features for Lyrics (5000-dim TF-IDF), Cover (512-dim Visual Moments), and Audio (128-dim Zero-Mask).
"""
import sys
import pickle
import numpy as np
...
def extract_visual_descriptor(img_path):
    """
    Extracts 512-dim spatial color histogram & gradient moments from cover image.
    """
    try:
        with Image.open(img_path) as img:
            ...
            # 1. 3D Color Histogram (8x8x4 = 256 bins)
            ...
            color_hist = np.concatenate([h_r, h_g, h_b, h_rg, h_gb])  # 8+8+8+144+100 = 268 bins

            # 2. Spatial 4-Quadrant Color Moments (4 quadrants x 3 channels x 4 moments = 48)
            moments = []
            h_mid, w_mid = 64, 64
            quads = [
            ...
            moments = np.array(moments, dtype=np.float32) # 48 dims

            # 3. Gradient Spatial Energy Moments (196 dims)
...
def extract_all_v3_features():
    print("=== RM-VMusic: Extracting Features for Dataset V3 (N = 5,569) ===")
    
    v3_csv_path = PROCESSED_DIR / "final_12class_metadata_v3.csv"
    ...
    # Fit vectorizer on non-empty lyrics texts
    ...
    # 4. Extract Audio Features (128-dim Zero-Masked for unmaterialized audio)
    print("Generating Audio Features (128-dim Zero-Masked)...")
    ...
    print(f"Saved audio_features_128.npy shape: {audio_feats.shape} (Zero-Masked: 100%)")

    print("\nFeature extraction for Dataset V3 completed successfully!")

if __name__ == "__main__":
    extract_all_v3_features()
